...
class BaseBoard(object):
    ...
    def rook(self, color):
        rook_mask = self.occupied_co[color] & self.rooks & ~self.promoted
        if rook_mask:
            return msb(rook_mask)

    def bishop(self, color):
        bishop_mask = self.occupied_co[color] & self.bishops & ~self.promoted
        if bishop_mask:
            return msb(bishop_mask)

    def knight(self, color):
        knight_mask = self.occupied_co[color] & self.knights & ~self.promoted
        if knight_mask:
            return msb(knight_mask)

    def pawn(self, color):
        pawn_mask = self.occupied_co[color] & self.pawns & ~self.promoted
        if pawn_mask:
            return msb(pawn_mask)        
